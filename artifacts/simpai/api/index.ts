...
async function getUserFromSheets(username: string) {
...
async function incrementQuota(username: string) {
  const n = username.trim().toLowerCase();
...
app.post("/api/auth/login", async (req, res) => {
  ...
  res.json({ success: true, username: user.username, kuotaSisa, kuotaMaks: user.kuotaMaks });
});
...
type UserRow = { username: string; password: string; kuotaMaks: number; kuotaTerpakai: number; status: string };
...
app.post("/api/admin/users", async (req, res) => {
  if (!isAdmin(req)) { res.status(403).json({ error: "Akses ditolak." }); return; }
  const { username, password, kuotaMaks, status } = req.body as { username?: string; password?: string; kuotaMaks?: number; status?: string };
  if (!username || !password) { res.status(400).json({ error: "Username dan password wajib diisi." }); return; }
  ...
  const result = await postSheets<{ success: boolean; error?: string }>({ action: "addUser", username: target, password, kuotaMaks: kuotaMaks ?? 50, status: status ?? "Aktif" });
...
app.patch("/api/admin/users/:username", async (req, res) => {
  ...
  if (!GSHEETS_URL) { res.status(400).json({ error: "Google Sheets belum terhubung." }); return; }
  const { password, kuotaMaks, kuotaTerpakai, status } = req.body as { password?: string; kuotaMaks?: number; kuotaTerpakai?: number; status?: string };
  const result = await postSheets<{ success: boolean; error?: string }>({ action: "updateUser", username: target, ...(password !== undefined && { password }), ...(kuotaMaks !== undefined && { kuotaMaks }), ...(kuotaTerpakai !== undefined && { kuotaTerpakai }), ...(status !== undefined && { status }) });
...
app.delete("/api/admin/users/:username", async (req, res) => {
  ...
  if (!GSHEETS_URL) { res.status(400).json({ error: "Google Sheets belum terhubung." }); return; }
  const result = await postSheets<{ success: boolean; error?: string }>({ action: "deleteUser", username: target });
...
app.post("/api/admin/cp-tp", async (req, res) => {
  ...
  const result = await postSheets<{ success: boolean; error?: string }>({ action: "updateCPTP", data: data as Record<string, unknown> });
...
app.get("/api/admin/cp-tp", async (_req, res) => {
  ...
  const data = await postSheets<{ data: unknown[] }>({ action: "getCPTP" });
...
app.get("/api/admin/kaldik", async (_req, res) => {
  if (!GSHEETS_URL) { res.json({ data: [] }); return; }
  const data = await postSheets<{ data: unknown[] }>({ action: "getKaldik" });
...
app.post("/api/admin/kaldik", async (req, res) => {
  ...
  if (!GSHEETS_URL) { res.json({ success: true, message: "Demo mode." }); return; }
  const result = await postSheets<{ success: boolean; error?: string }>({ action: "updateKaldik", data: data as Record<string, unknown> });
...
async function generateWithRetry(prompt: string, clientKey?: string): Promise<string> {
  ...
  for (const model of MODELS) {
    try {
      ...
      if (retryable) { lastError = err instanceof Error ? err : new Error(msg); continue; }
      throw err;
    }
  }
...
app.post("/api/ai/generate", async (req, res) => {
  try {
    const { prompt, clientKey } = req.body as { prompt?: string; clientKey?: string };
